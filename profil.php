          ...
          elseif(!empty($_POST['adresse']) AND !empty ($_POST['password']) AND !empty ($_POST['motdepasse']) ){
          	# code...
                  	  ...
                  	  if($_POST['password']==$_POST['motdepasse']) {
                  	  	if ($_POST['adresse']==$_SESSION['adresseAd']) {
                  	  		# code...
                        $newadresse=$_POST['adresse'];
                        
                        $newpassword=$_POST['password'];
                        $requser = $conn ->prepare("UPDATE connexionAd SET adresse = ? WHERE id = ?");
                        $requser->execute(array($newadresse,$_SESSION['idAd']));

                        $insertmp = $conn->prepare("UPDATE connexionAd SET password= ? WHERE id = ?"); //modifier la ligne de la base 
                       $insertmp->execute(array($newpassword, $_SESSION['idAd']));
                  ...
                  else{ $reqmail = $conn->prepare("SELECT * FROM connexionAd WHERE adresse = ?");//toujours premiérerement il faut préparer la requét
