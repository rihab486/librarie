...
elseif (!empty($_POST['titre']) AND !empty($_POST['genre']) AND !empty($_POST['number']) AND !empty($_FILES['fichier']) AND !empty($_POST['resume'])  ) {
            ...
            $reqmail = $conn->prepare("SELECT * FROM deposerDocument WHERE adresse =? AND titre=? AND genre=? namefichier=? AND file_url=? AND resume=? ");//toujours premiérerement il faut préparer la requét
                   $reqmail->execute(array($_SESSION['adresseAd'],$_POST['titre'],$_POST['genre'],$file_name,$file_dest, $_POST['resume']));
...
$stmt->bindParam(':aadresse', $_SESSION['adresseAd'], PDO::PARAM_STR);
...
<head>
    ...
    <title>Document</title>
        ...
        <div class="title">déposez votre document</div>
         <form method="POST" action="deposerDocAd.php" enctype="multipart/form-data">
